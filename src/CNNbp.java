...
public class CNNbp extends Structure {
    ...
    public static network CNNbp(String[][] architecture, network convnet, int[][] batch_y){
        ...
        for (int i = 0; i < convnet.o.length; i++) {
            for (int j = 0; j < convnet.o[0].length; j++) {
                dotprod_1[i][j]=convnet.o[i][j]*(1-convnet.o[i][j]);
            }
        }
        double[][] od = new double[convnet.o.length][convnet.o[0].length];
        for (int i = 0; i < convnet.o.length; i++) {
            for (int j = 0; j < convnet.o[0].length; j++) {
                od[i][j]=e[i][j]*dotprod_1[i][j];
            }
        }
        convnet.od=od;
            ...
            if("c".equals(architecture[0][i])){
                ...
                for (int j = 0; j < num_a; j++) {
                    ...
                    for (int k = 0; k < size_a3; k++) {
                        for (int l = 0; l < size_a2; l++) {
                            for (int m = 0; m < size_a1; m++) {
                                product_1[m][l][k]=((double[][][]) convnet.layers.get(i).a.get(0).a_list.get(j))[m][l][k]*(1.0-((double[][][]) convnet.layers.get(i).a.get(0).a_list.get(j))[m][l][k]);
                            }
                        }
                    }
                    ...
                    for (int k = 0; k < size_a3; k++) {
                        for (int l = 0; l < size_a2; l++) {
                            for (int m = 0; m < size_a1; m++) {
                                product_2[m][l][k]=product_1[m][l][k]*expand[m][l][k]/((double) sub_scale*sub_scale);
                            }
                        }
                    }
        ...
        for (int i = 0; i < convnet.od.length; i++) {
            double od_row_sum = 0;
            for (int j = 0; j < convnet.od[0].length; j++) {
                od_row_sum = od_row_sum+convnet.od[i][j];
            }
            dffb[i]=od_row_sum/(double)convnet.od[0].length;
        }
        convnet.dffb=dffb;
